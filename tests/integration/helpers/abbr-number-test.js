...
module('scott Integration | Helper | abbr-number', function(hooks) {
  ...
  test('abbreviates correctly', async function(assert) {
    ...
    assert.equal(this.element.textContent.trim(), '999M', '999000000 is abbreviated correctly'); // passes 3.0, fails 3.1.1 undefined
    ...
    assert.equal(this.element.textContent.trim(), '1B', '1 bil is abbreviated correctly'); // passes 3.0, fails 3.1.1 - undefined
  });
});
